...
define('INPARK_RESERVA_VERSION', '1.0.7');
...
/**
 * Enqueue public scripts - força no-cache
 */
function inpark_reserva_enqueue() {
    // Força reload sem cache
    $version = INPARK_RESERVA_VERSION . '.' . time();
    
    wp_enqueue_script('inpark-reserva-js', INPARK_RESERVA_URL . 'public/js/inpark-reserva.js', array(), $version, true);
    $opts = get_option('inpark_reserva_settings', array());
    $localized = array(
        'ajaxurl' => admin_url('admin-ajax.php'),
        'precos'  => array(
            'meio_dia' => floatval($opts['preco_meio_dia'] ?? 220),
            'dia_inteiro' => floatval($opts['preco_dia_inteiro'] ?? 280),
            'hora_extra_meio' => floatval($opts['hora_extra_meio'] ?? 49.50),
            'hora_extra_inteiro' => floatval($opts['hora_extra_inteiro'] ?? 63.00),
            'taxa_limpeza' => floatval($opts['taxa_limpeza'] ?? 40),
        ),
    ...
    wp_enqueue_style('inpark-reserva-css', INPARK_RESERVA_URL . 'public/style.css', array(), $version);
}
