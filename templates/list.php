<?php
if ( ! defined( 'ABSPATH' ) ) {
    exit;
}
?>
<div class="oportunidades-list">
    <?php if ( empty( $records ) ) : ?>
        <p><?php esc_html_e( 'Não foram encontradas oportunidades.', 'oportunidades' ); ?></p>
    <?php else : ?>
        <table class="oportunidades-table">
            <thead>
                <tr>
                    <th><?php esc_html_e( 'Título', 'oportunidades' ); ?></th>
                    <th><?php esc_html_e( 'Entidade', 'oportunidades' ); ?></th>
                    <th><?php esc_html_e( 'Valor', 'oportunidades' ); ?></th>
                    <th><?php esc_html_e( 'Prazo', 'oportunidades' ); ?></th>
                    <th><?php esc_html_e( 'Categorias', 'oportunidades' ); ?></th>
                    <th><?php esc_html_e( 'Link', 'oportunidades' ); ?></th>
                </tr>
            </thead>
            <tbody>
            <?php foreach ( $records as $record ) : ?>
                <tr>
                    <td><?php echo esc_html( $record['title'] ); ?></td>
                    <td><?php echo esc_html( $record['awarding_entity'] ); ?></td>
                    <td><?php echo esc_html( null !== $record['value_normalized'] ? number_format_i18n( $record['value_normalized'], 2 ) : '—' ); ?></td>
                    <td><?php echo esc_html( $record['deadline_date'] ? get_date_from_gmt( $record['deadline_date'], 'd/m/Y' ) : '—' ); ?></td>
                    <td><?php echo esc_html( implode( ', ', (array) $record['categories'] ) ); ?></td>
                    <td><a href="<?php echo esc_url( $record['url'] ); ?>" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Abrir', 'oportunidades' ); ?></a></td>
                </tr>
            <?php endforeach; ?>
            </tbody>
        </table>
    <?php endif; ?>
</div>
